...
import com.acmedcare.framework.applet.integrate.api.endpoints.NasEndpoint;
  ...
  public Set<Class<?>> getClasses() {
    ...
    Set<Class<?>> classes =
        Sets.newHashSet(
            NasEndpoint.class,
            AuthEndpoint.class,
            PrincipalEndpoint.class,
            AppletsExceptionMapper.class);

    classes.addAll(AppletsContext.loadExtensionResourceClasses());

    log.info(
        "[==Applet RESTEasy==] config-ed resource 、provider set size :{} - {}",
        classes.size(),
        classes.toArray());
