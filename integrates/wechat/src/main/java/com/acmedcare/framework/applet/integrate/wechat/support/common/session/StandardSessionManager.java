...
import com.acmedcare.framework.applet.integrate.wechat.support.common.util.res.StringManager;
...
public class StandardSessionManager implements WxSessionManager, InternalSessionManager {
  ...
  /**
   * Frequency of the session expiration, and related manager operations. Manager operations will be
   * done once for the specified amount of backgrondProcess calls (ie, the lower the amount, the
   * most often the checks will occur).
   */
  ...
  public InternalSession createSession(String sessionId) {
    ...
    if ((this.maxActiveSessions >= 0) && (getActiveSessions() >= this.maxActiveSessions)) {
      ...
      throw new TooManyActiveSessionsException(
          SM.getString("sessionManagerImpl.createSession.tmase"), this.maxActiveSessions);
    }
                ...
                public void run() {
                  while (true) {
                    try {
                      ...
                      StandardSessionManager.this.log.error(
                          "SessionManagerImpl.backgroundProcess error", e);
                    }
                  }
                }
              });
  ...
  /**
   * Return the set of active Sessions associated with this Manager. If this Manager has no active
   * Sessions, a zero-length array is returned.
   */
  @Override
