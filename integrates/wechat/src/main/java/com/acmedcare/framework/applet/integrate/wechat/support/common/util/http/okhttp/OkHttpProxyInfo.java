...
/** Created by ecoolper on 2017/4/26. Proxy information. */
public class OkHttpProxyInfo {
  ...
  public static OkHttpProxyInfo socks5Proxy(
      String proxyAddress, int proxyPort, String proxyUser, String proxyPassword) {
  ...
  /** Returns proxy user name or <code>null</code> if no authentication required. */
  ...
  public Proxy getProxy() {
    ...
    if (getProxyType().equals(ProxyType.SOCKS5)) {
      ...
      proxy =
          new Proxy(Proxy.Type.DIRECT, new InetSocketAddress(getProxyAddress(), getProxyPort()));
    }
    return proxy;
  }
