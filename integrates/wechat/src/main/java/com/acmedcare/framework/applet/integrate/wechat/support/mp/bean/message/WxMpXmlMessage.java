...
import com.acmedcare.framework.applet.integrate.wechat.support.common.util.XmlUtils;
...
import com.acmedcare.framework.applet.integrate.wechat.support.mp.util.crypto.WxMpCryptUtil;
import com.acmedcare.framework.applet.integrate.wechat.support.mp.util.json.WxMpGsonBuilder;
import com.acmedcare.framework.applet.integrate.wechat.support.mp.util.xml.XStreamTransformer;
...
public class WxMpXmlMessage implements Serializable {
   ...
   * 审核结果，成功succ 或失败fail.
   *
   * <p>在商品审核结果推送时，verify_ok表示审核通过，verify_not_pass表示审核未通过。
   */
  @XStreamAlias("Result")
  ...
  ///////////////////////////////////////
  /**
   * 资质认证成功/名称认证成功: 有效期 (整形)，指的是时间戳，将于该时间戳认证过期. 年审通知: 有效期 (整形)，指的是时间戳，将于该时间戳认证过期，需尽快年审 认证过期失效通知: 有效期
   * (整形)，指的是时间戳，表示已于该时间戳认证过期，需要重新发起微信认证
   */
  ...
  /**
   * 用户的实时地理位置信息（目前只精确到省一级），可在国家统计局网站查到对应明细：
