package com.acmedcare.framework.applet.integrate.wechat.support.common.util.http.apache;
...
public class DefaultApacheHttpClientBuilder implements ApacheHttpClientBuilder {
   ...
   *
   * <p>设置为零时不超时,一直等待. 设置为负数是使用系统默认设置(非上述的3000ms的默认值,而是httpclient的默认设置).
   *
   ...
   *
   * <p>设置为零时不超时,一直等待. 设置为负数是使用系统默认设置(非上述的5000ms的默认值,而是httpclient的默认设置).
   *
  ...
  private synchronized void prepare() {
    ...
    connectionManager.setDefaultSocketConfig(
        SocketConfig.copy(SocketConfig.DEFAULT).setSoTimeout(this.soTimeout).build());

    this.idleConnectionMonitorThread =
  ...
  private static class SingletonHolder {
    private static final DefaultApacheHttpClientBuilder INSTANCE =
        new DefaultApacheHttpClientBuilder();
  }
  ...
  public static class IdleConnectionMonitorThread extends Thread {
    ...
    public IdleConnectionMonitorThread(
        HttpClientConnectionManager connMgr, int idleConnTimeout, int checkWaitTime) {
