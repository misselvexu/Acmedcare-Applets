...
import com.acmedcare.framework.applet.integrate.wechat.support.open.api.*;
...
import com.acmedcare.framework.applet.integrate.wechat.support.open.bean.WxOpenCreateResult;
import com.acmedcare.framework.applet.integrate.wechat.support.open.bean.WxOpenMaCodeTemplate;
import com.acmedcare.framework.applet.integrate.wechat.support.open.bean.auth.WxOpenAuthorizationInfo;
import com.acmedcare.framework.applet.integrate.wechat.support.open.bean.message.WxOpenXmlMessage;
...
public class WxOpenComponentServiceImpl implements WxOpenComponentService {
  ...
  private static final Map<String, WxOpenMaService> WX_OPEN_MA_SERVICE_MAP =
      new ConcurrentHashMap<>();
  private static final Map<String, WxMpService> WX_OPEN_MP_SERVICE_MAP = new ConcurrentHashMap<>();
  private static final Map<String, WxOpenFastMaService> WX_OPEN_FAST_MA_SERVICE_MAP =
      new ConcurrentHashMap<>();
  ...
  public WxMpService getWxMpServiceByAppid(String appId) {
    ...
    if (wxMpService == null) {
      synchronized (WX_OPEN_MP_SERVICE_MAP) {
        ...
        if (wxMpService == null) {
          wxMpService =
              new WxOpenMpServiceImpl(
                  this, appId, getWxOpenConfigStorage().getWxMpConfigStorage(appId));
  ...
  public WxOpenMaService getWxMaServiceByAppid(String appId) {
    ...
    if (wxOpenMaService == null) {
      synchronized (WX_OPEN_MA_SERVICE_MAP) {
        ...
        if (wxOpenMaService == null) {
          wxOpenMaService =
              new WxOpenMaServiceImpl(this, appId, getWxOpenConfigStorage().getWxMaConfig(appId));
  ...
  public WxOpenFastMaService getWxFastMaServiceByAppid(String appId) {
    ...
    if (fastMaService == null) {
      synchronized (WX_OPEN_FAST_MA_SERVICE_MAP) {
        ...
        if (fastMaService == null) {
          fastMaService =
              new WxOpenFastMaServiceImpl(
                  this, appId, getWxOpenConfigStorage().getWxMaConfig(appId));
  ...
  public String getComponentAccessToken(boolean forceRefresh) throws WxErrorException {
    ...
    if (this.getWxOpenConfigStorage().isComponentAccessTokenExpired() || forceRefresh) {
      ...
      jsonObject.addProperty(
          "component_verify_ticket", getWxOpenConfigStorage().getComponentVerifyTicket());

      String responseContent =
          this.getWxOpenService().post(API_COMPONENT_TOKEN_URL, jsonObject.toString());
      WxOpenComponentAccessToken componentAccessToken =
          WxOpenComponentAccessToken.fromJson(responseContent);
  ...
  private String post(String uri, String postData, String accessTokenKey) throws WxErrorException {
    ...
    String uriWithComponentAccessToken =
        uri + (uri.contains("?") ? "&" : "?") + accessTokenKey + "=" + componentAccessToken;
    try {
   ...
   * @throws WxErrorException
   */
  private String createPreAuthUrl(
      String redirectURI, String authType, String bizAppid, boolean isMobile)
      throws WxErrorException {
    JsonObject jsonObject = new JsonObject();
    jsonObject.addProperty("component_appid", getWxOpenConfigStorage().getComponentAppId());
    ...
    StringBuilder preAuthUrl =
        new StringBuilder(
            String.format(
                (isMobile ? COMPONENT_MOBILE_LOGIN_PAGE_URL : COMPONENT_LOGIN_PAGE_URL),
  ...
  public String route(final WxOpenXmlMessage wxMessage) throws WxErrorException {
    ...
    if (StringUtils.equalsAnyIgnoreCase(
      ...
      WxOpenQueryAuthResult queryAuth =
          wxOpenService.getWxOpenComponentService().getQueryAuth(wxMessage.getAuthorizationCode());
      if (queryAuth == null
          || queryAuth.getAuthorizationInfo() == null
          || queryAuth.getAuthorizationInfo().getAuthorizerAppid() == null) {
        throw new NullPointerException("getQueryAuth");
      }
      return "success";
    }
    // 快速创建小程序
    if (StringUtils.equalsIgnoreCase(wxMessage.getInfoType(), "notify_third_fasteregister")
        && wxMessage.getStatus() == 0) {
      WxOpenQueryAuthResult queryAuth =
          wxOpenService.getWxOpenComponentService().getQueryAuth(wxMessage.getAuthCode());
      if (queryAuth == null
          || queryAuth.getAuthorizationInfo() == null
          || queryAuth.getAuthorizationInfo().getAuthorizerAppid() == null) {
        throw new NullPointerException("getQueryAuth");
  ...
  public WxOpenQueryAuthResult getQueryAuth(String authorizationCode) throws WxErrorException {
    ...
    WxOpenQueryAuthResult queryAuth =
        WxOpenGsonBuilder.create().fromJson(responseContent, WxOpenQueryAuthResult.class);
  ...
  public WxOpenAuthorizerListResult getAuthorizerList(int begin, int len) throws WxErrorException {
    ...
    WxOpenAuthorizerListResult ret =
        WxOpenGsonBuilder.create().fromJson(responseContent, WxOpenAuthorizerListResult.class);
  ...
  public String getAuthorizerAccessToken(String appId, boolean forceRefresh)
    ...
    if (this.getWxOpenConfigStorage().isAuthorizerAccessTokenExpired(appId) || forceRefresh) {
      ...
      WxOpenAuthorizerAccessToken wxOpenAuthorizerAccessToken =
          WxOpenAuthorizerAccessToken.fromJson(responseContent);
  ...
  public WxMpOAuth2AccessToken oauth2getAccessToken(String appId, String code)
    ...
    String url =
        String.format(
            OAUTH2_ACCESS_TOKEN_URL, appId, code, getWxOpenConfigStorage().getComponentAppId());
  ...
  @Override
  public WxMpOAuth2AccessToken oauth2refreshAccessToken(String appId, String refreshToken)
      throws WxErrorException {
    String url =
        String.format(
  ...
  public String oauth2buildAuthorizationUrl(
      String appId, String redirectURI, String scope, String state) {
  ...
  public WxMaJscode2SessionResult miniappJscode2Session(String appId, String jsCode)
    ...
    String url =
        String.format(
            MINIAPP_JSCODE_2_SESSION, appId, jsCode, getWxOpenConfigStorage().getComponentAppId());
    String responseContent = get(url);
  ...
  public List<WxOpenMaCodeTemplate> getTemplateDraftList() throws WxErrorException {
    ...
    JsonObject response =
        JSON_PARSER.parse(StringUtils.defaultString(responseContent, "{}")).getAsJsonObject();
    boolean hasDraftList = response.has("draft_list");
    if (hasDraftList) {
      return WxOpenGsonBuilder.create()
          .fromJson(
              response.getAsJsonArray("draft_list"),
  ...
  public List<WxOpenMaCodeTemplate> getTemplateList() throws WxErrorException {
    ...
    if (hasDraftList) {
      return WxOpenGsonBuilder.create()
          .fromJson(
              response.getAsJsonArray("template_list"),
              new TypeToken<List<WxOpenMaCodeTemplate>>() {}.getType());
    } else {
      return null;
    }
  ...
  @Override
  public WxOpenResult fastRegisterWeapp(
      String name,
      String code,
      String codeType,
  ...
  @Override
  public WxOpenResult fastRegisterWeappSearch(
      String name, String legalPersonaWechat, String legalPersonaName) throws WxErrorException {
    JsonObject jsonObject = new JsonObject();
    jsonObject.addProperty("name", name);
    jsonObject.addProperty("legal_persona_wechat", legalPersonaWechat);
    jsonObject.addProperty("legal_persona_name", legalPersonaName);
