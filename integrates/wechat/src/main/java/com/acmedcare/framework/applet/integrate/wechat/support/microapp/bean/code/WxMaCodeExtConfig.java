...
public class WxMaCodeExtConfig implements Serializable {
  ...
  /** 配置 ext.json 是否生效. 必填：是 */
  private boolean extEnable;
  /** 配置 extAppid. 必填：是 */
  private String extAppid;
  /** 开发自定义的数据字段. 必填：否 */
  private Object ext;
  /** 单独设置每个页面的 json. 必填：否 key: page 名称，如 pages/logs/logs value: page 配置 */
  private Map<String, PageConfig> extPages;
  /** 是否直接提交到待审核列表. 必填：否 */
  private Boolean directCommit;
  /** 设置页面路径（同 app.json 相同的字段，填写会覆盖 app.json）. 必填：否 */
  private List<String> pages;
  /** 设置默认页面的窗口表现（同 app.json 相同的字段，填写会覆盖 app.json） 必填：否 */
  private PageConfig window;
  /** 设置各种网络请求的超时时间（同 app.json 相同的字段，填写会覆盖 app.json） 必填：否 */
  private NetworkTimeout networkTimeout;
  /** 设置是否开启 debug 模式（同 app.json 相同的字段，填写会覆盖 app.json） 必填：否 */
  private Boolean debug;
  /** 底部 tab 栏的表现. 必填：否 */
  private TabBar tabBar;

  /** page.json 配置，页面配置 文档：https://mp.weixin.qq.com/debug/wxadoc/dev/framework/config.html */
  @Data
  @Builder
  public static class PageConfig {
    ...
    /** 设置为 true 则页面整体不能上下滚动；只在 page.json 中有效，无法在 app.json 中设置该项. 默认：false */
    private Boolean disableScroll;
    /** 页面上拉触底事件触发时距页面底部距离，单位为px. 默认：50 */
  ...
  public static class NetworkTimeout {
    /** wx.request的超时时间，单位毫秒，默认为：60000. 必填：否 */
    private Integer request;
    /** wx.connectSocket的超时时间，单位毫秒，默认为：60000. 必填：否 */
    private Integer connectSocket;
    /** wx.uploadFile的超时时间，单位毫秒，默认为：60000. 必填：否 */
    private Integer uploadFile;
    /** wx.downloadFile的超时时间，单位毫秒，默认为：60000. 必填：否 */
