 ...
 *
 * <p>如果代码中已经有配置，则配置的合并规则为：除了pages和tabBar.list直接覆盖原配置，其他都为插入或同级覆盖。
 *
 * @author yqx
 * @date 2018/9/13
 */
...
public class WxMaOpenCommitExtInfo implements Serializable {
  ...
  public void addExt(String key, String value) {
    ...
    if (StringUtils.isNoneBlank(key, value)) extMap.put(key, value);
  }
  ...
  public void addExtPage(String pagePath, WxMaOpenPage page) {
    if (extPages == null) extPages = new HashMap<>();
    if (StringUtils.isNotBlank(pagePath) && page != null) extPages.put(pagePath, page);
  }
  ...
  public void addPage(String pagePath) {
    ...
    if (StringUtils.isNotBlank(pagePath)) pageList.add(pagePath);
  }
}
